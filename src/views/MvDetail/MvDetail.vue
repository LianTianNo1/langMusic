      ...
      simiMVAPI({ mvid: id }).then((res) => {
        ...
        for (let i = 0; i < this.recommendMvs.length; i++) {
          if (this.recommendMvs[i].playCount >= 100000) {
            this.recommendMvs[i].playCount =
              parseInt(this.recommendMvs[i].playCount / 10000) + "万";
          }
          const duration = this.recommendMvs[i].duration;
